...
var (
	cmd = &cobra.Command{
		Use:     "Client Gateway",
		Short:   "Gateway of Client service",
		Long:    fmt.Sprintf("Gateway of Client service. Build at %s", BuildTime),
		Version: Version,
		Run:     onCmdAction,
	}
)
...
func init() {
	cmd.PersistentFlags().IntVarP(&port, "port", "p", 7883, "http server port")
}
