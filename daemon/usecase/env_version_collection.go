...
func NewEnvVersionCollection(config EnvVersionCollectionConfig) (*EnvVersionCollection, error) {
	if config.DomainAdapter == nil ||
		config.EnvId == nil {
		return nil, errors.Wrap(daemon.ErrParamsInvalid, "invalid version collection params")
	}
...
type EnvVersionCollection struct {
	...
	versionList       daemon.VersionList
...
func (e *EnvVersionCollection) ReleaseVersion(params VersionReleaseParams) error {
	...
	if rawAppVersion == nil ||
		packageFileKey == nil {
		return errors.Wrapf(daemon.ErrParamsInvalid, "appVersion, compatAppVersion nor packageFileKey can't be empty")
	}
	...
	if appVersionErr != nil {
		return errors.Wrapf(daemon.ErrParamsInvalid, "parse appVersion failed")
	}

	compatAppVersion, compatVersionErr := semver.ParseVersion(string(rawCompatAppVersion))
	if compatVersionErr != nil {
		return errors.Wrapf(daemon.ErrParamsInvalid, "parse compatAppVersion failed")
	}

	if compatAppVersion.StageSafetyLooseCompare(appVersion) == semver.CompareLargeFlag {
		return errors.Wrapf(daemon.ErrParamsInvalid, "compatAppVersion shall not larger than appVersion")
	}
	...
	if versionAvailable, versionAvailableErr := e.domain.IsVersionAvailable([]byte(e.envId), []byte(appVersion.String())); versionAvailableErr != nil {
		...
		return errors.Wrapf(daemon.ErrVersionExisted, "envId:%s, appVersion:%s", e.envId, appVersion.String())
	}

	newVersion := &daemon.Version{
		EnvId:            e.envId,
...
func (e *EnvVersionCollection) ListVersions() (daemon.VersionList, error) {
	...
	if versionList == nil {
		versionList = make(daemon.VersionList, 0)
	}
...
func newVersionCompatEntry(version *daemon.Version) *versionCompatEntry {
