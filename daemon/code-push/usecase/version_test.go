...
func TestVersion(t *testing.T) {
	t.Run("invalid params", func(t *testing.T) {
		{
			...
			assert.True(t, errors.Is(err, code_push.ErrParamsInvalid))
		}
		{
	...
	t.Run("release version on valid env", func(t *testing.T) {
		...
		t.Run("failed with invalid params", func(t *testing.T) {
			...
			assert.True(t, errors.Is(err, code_push.ErrParamsInvalid), "appVersion required")
			...
			assert.True(t, errors.Is(err, code_push.ErrParamsInvalid), "packageFileKey required")
		})
		...
		t.Run("release with existed appVersion", func(t *testing.T) {
			...
			assert.True(t, errors.Is(err, code_push.ErrVersionExisted), "failed to release existed app version")
		})
		...
		t.Run("release when compatAppVersion is large than appVersion", func(t *testing.T) {
			...
			for _, item := range data {
				...
				assert.True(t, errors.Is(err, code_push.ErrParamsInvalid), "compatAppVersion <= appVersion")
			}
		})
	})
	...
	t.Run("fetch on non existed env", func(t *testing.T) {
		...
		assert.True(t, errors.Is(getErr, code_push.ErrEnvNotFound))
		assert.Nil(t, ver)

		list, listErr := useCase.ListVersions([]byte("not existed env"))
		assert.True(t, errors.Is(listErr, code_push.ErrEnvNotFound))
		assert.Nil(t, list)
	})
}
